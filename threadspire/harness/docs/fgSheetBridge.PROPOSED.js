// ============================================================================
// PROPOSED replacement for velo/public/fgSheetBridge.js — paste this whole file.
//
// TWO CHANGES, and the FIRST one is the urgent one:
//
//   1. combat-declare now passes `places` through. It did not, and the field list here is
//      explicit, so a placement's squares died at THIS hop - one above the collection F7
//      spent its whole finding on. The backend has had `places` on the write and on both
//      reads since 24bfb6a and has been receiving undefined every time. If the live Ground
//      readout on the LoreMaster's board says "empty - the squares did not survive the
//      trip", this is why, and this line is the fix.
//
//      I cannot read the pasted Wix copy, so I cannot tell whether it already carries this
//      or whether the repo copy is simply stale. The readout was built to answer exactly
//      that question, so it is worth one look at a real declare before assuming either.
//
//   2. combat-sync now passes `placedAck`, the sheet's acknowledgement that it has spent a
//      placement the loremaster resolved. Pairs with combat.web.PROPOSED.js.
// ============================================================================
  ...
  if (m.type === 'ready') {
    ...
    try {
      await api.syncCombatPlayer(m.charId || getId(), {
        ...
        // the sheet's acknowledgement that it has spent a resolved placement; see the
        // note on placedAck in combat.web.js
        placedAck: m.placedAck
      });
    } catch (e) { ok = false; }
    ...
    try {
      await api.saveCombatDeclare(m.charId || getId(), {
        ...
        // THE SQUARES A PLACEMENT WAS DECLARED ON.
        //
        // This is the hop F7 did not look at. F7 added `places` to saveCombatDeclare and
        // to both of its reads, three sites, and called the round trip complete - but the
        // declare does not reach saveCombatDeclare directly. It comes through HERE, and
        // this handler builds an explicit field list. A field not named in that list is
        // not passed on, so `d.places` arrived at the backend undefined and the collection
        // stored "[]" no matter how many squares the player tapped.
        //
        // Which means the fix F7 shipped could look complete in the diff, pass every
        // check the repo can make, and still put nothing on the ground - the same shape of
        // miss the finding itself is about, one hop further up. A field list that has to
        // be kept in step with a payload is a thing that will drift again; it is left as a
        // list rather than a spread only because that is how every other handler in this
        // file is written and one inconsistent handler is its own hazard.
        places: m.places
      });
    } catch (e) { ok = false; }
